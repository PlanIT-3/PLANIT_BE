...
public class AccountService {
					...
					for (ResSecuritiesAccount securitiesAccount : securitiesData.getData()) {
						...
						if (accountProductData != null && accountProductData.getData() != null && 
							...
							if (!memberProducts.isEmpty()) {
								memberProductMapper.insertAll(memberProducts);
	...
	private List<MemberProduct> convertToMemberProducts(List<ResProductItem> resProductItems, Long memberId, 
		...
		return resProductItems.stream().map(item -> {
			...
			memberProduct.setProductTypeCode(item.getResProductTypeCd());
			memberProduct.setProductType(item.getResProductType());
			...
			// 상품명으로 product_id 조회, 없으면 1로 설정
			Long productId = productMapper.selectProductIdByItemName(item.getResItemName());
			memberProduct.setProductId(productId != null ? productId : 1L);
